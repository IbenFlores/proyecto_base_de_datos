...
(2, 3),    -- Shakira colabora en YHLQMDLG (ejemplo de colaboraci�n)
...
(6, 2, 2, 2, 'Estoy Aqu�', 'Shakira', 231000, 4659200, 129),
(7, 2, 2, 2, 'Pies Descalzos, Sue�os Blancos', 'Shakira', 220000, 4433920, 129),
(8, 2, 2, 2, 'D�nde Est�s Coraz�n', 'Shakira', 230000, 4638720, 129),
...
(14, 3, 1, 3, 'Si Veo a Tu Mam�', 'Bad Bunny', 190000, 3834880, 99),
(15, 3, 1, 3, 'Soli�', 'Bad Bunny', 180000, 3630080, 99),
...
(47, 10, 1, 3, 'Pienso en Tu Mir�', 'Rosalia', 189000, 3814400, 99),
...
(55, 13, 1, 3, 'Tit� Me Pregunt�', 'Bad Bunny', 243000, 4904960, 99),
...
(64, 17, 1, 3, 'Sigo Extra��ndote', 'J Balvin', 199000, 4014080, 99),
...
(73, 2, 2, 2, 'Un D�a Sin Ti', 'Shakira', 240000, 4843520, 129),
(74, 3, 1, 3, 'Hablamos Ma�ana', 'Bad Bunny', 235000, 4741120, 99),
...
(93, 2, 2, 2, 'Antolog�a', 'Shakira', 263000, 5304320, 129),
...
-- Insertar datos en la tabla usuarios (10 registros, IDs expl�citas)
SET IDENTITY_INSERT usuarios ON;
INSERT INTO usuarios (id, nombre, correo, contrase�a, fecha_registro) VALUES
...
-- Insertar datos en la tabla planes (3 registros, IDs expl�citas)
SET IDENTITY_INSERT planes ON;
INSERT INTO planes (id, nombre, precio, duracion_meses) VALUES
(1, 'Gratis', 0.00, NULL), -- Sin duraci�n fija
(2, 'Est�ndar', 7.99, 1),
(3, 'Premium Anual', 79.99, 12);
SET IDENTITY_INSERT planes OFF;

-- Insertar datos en la tabla suscripciones (10 registros, IDs expl�citas)
-- NOTA: Las fechas_fin se calculan a partir de la fecha actual para simular estados de suscripci�n.
-- La funci�n GETDATE() se usa para la fecha de inicio actual si no se especifica.
...
(2, 2, 2, GETDATE(), DATEADD(month, 1, GETDATE())), -- Carlos: Est�ndar, activa
...
(5, 5, 2, '2024-01-15', '2024-02-15'), -- Sofia: Est�ndar, expirada
(6, 6, 1, GETDATE(), NULL), -- Diego: Gratis
(7, 7, 3, GETDATE(), DATEADD(year, 1, GETDATE())), -- Mariana: Premium, activa
(8, 8, 2, '2023-09-01', '2023-10-01'), -- Ricardo: Est�ndar, expirada
...
-- Insertar datos en la tabla playlists (15 registros, IDs expl�citas)
...
(3, 'Relajaci�n Indie', 3, '2024-02-15'),
(4, 'Reggaeton Party', 4, '2024-04-10'),
(5, 'Metal Cl�sico', 5, '2023-12-01'),
...
(13, 'Cl�sicos de Rock', 9, '2024-02-28'),
...
-- ###############################################################
-- #           INSERCI�N DE DATOS PARA TABLA SEGUIMIENTOS        #
-- ###############################################################
...
(2, 1, DATEADD(day, -5, GETDATE())), -- Carlos sigue a Andrea (hace 5 d�as)
